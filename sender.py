...
class RDTSender:
    ...
    def is_corrupted(reply):
        ...
        """
        # TODO provide your own implementation
        #we need to check if checksum(reply)==checksum() 
        #return False
    ...
    def rdt_send(self, process_buffer):
        ...
        for data in process_buffer:
            ...
            print(RDTSender.is_corrupted(reply))
            print(RDTSender.is_expected_seq(reply, self.sequence))
            while (RDTSender.is_corrupted(reply)) or (not RDTSender.is_expected_seq(reply, self.sequence)):
               clone1 = RDTSender.clone_packet(clone)
               reply = self.net_srv.udt_send(clone1)
